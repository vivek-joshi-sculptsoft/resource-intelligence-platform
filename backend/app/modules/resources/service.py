...
async def update_resource(
    ...
    **fields: Any,
    ...
    await db.flush()
    # reporting_manager was already eager-loaded by get_resource() above, so the
    # identity map won't refresh it on the re-query below unless explicitly expired.
    db.expire(resource, ["reporting_manager"])
    result = await db.execute(
        select(Resource)
        .options(
            selectinload(Resource.tags),
